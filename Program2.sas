...
BALANCING FOR RACE and imposing conditions on average age for participants in each group.
...
********************************************************************************/;
    	...
    	data DDE.DM_&i. (replace=yes);
            ...
            UniqueID=USUBJID;
/****************************************************************************** 
Ideally Age should not be missing.  However, impute (or remove upstream) missing values as needed.
********************************************************************************/;
            if missing(age) then age=65;
    	run;
...
********************************************************************************/;
    		...
    		read data DDE.DM_&i. into RACE=[USUBJID] AGE ;

		/* Decision variables */
    		var x{RACE} binary;

			/* Objective: maximize values */
    		max TotalValue = sum{i in RACE} (age[i] ) * x[i];
...
********************************************************************************/;
        ...
        /* Margin of Errors are defined with constraints so .98 and 1.02 for example means a 5 percent margin of error in either direction */
            ...
            con VAL2LimitLow: &val1agg. * &val2agg. * 0.96 <= sum{i in RACE} age[i] * x[i];
            con VAL2LimitHigh: sum{i in RACE} dmdy[i] * x[i] <= &val1agg. * &val2agg. * 1.04 ;
...
********************************************************************************/;	
		...
		/* Output the selected observation IDs and the binary to a dataset named for the sub-group being processed */
			create data DDE.opt_&i.(replace=yes rename = (i=USUBJID)) from [i] x AGE ;
		quit;
  	%end;
%mend;
