...
BALANCING FOR RACE and imposing conditions on the DMDY variable.
...
/****************************************************************************** 
Create a macro list of RACE variable
/******************************************************************************/; 
...
/* Create a macro that takes the list of groups, creates unique datasets named for those groups, 
and splits the original dataset into the respective dataset. Then the optimization algorithm is run
against each by group dataset. */

%macro write_by_group (lib=dd, tableName=DM_NEW, OPT_VAR = RACE);
/* Defines the indicator macro and the macro for the length of the loop (i and n respectively) */
	%local i in group;
	...
	%do i = 1 %to &n.;
    	...
    	/* Read data into parameters */
    		...
    		read data DDE.DM_&i. into RACE=[USUBJID] DMDY;

		/* Decision variables */
    		var x{RACE} binary;

			/* Objective: maximize values */
    		max TotalValue = sum{i in RACE} (dmdy[i] * -1 ) * x[i];
		...
		/* Margin of Errors are defined with constraints so .98 and 1.02 for example means a 5 percent margin of error in either direction */
            ...
            con VAL2LimitLow: &val1agg. * &val2agg. * 0.98 >= sum{i in RACE} dmdy[i] * x[i];
            con VAL2LimitHigh: sum{i in RACE} dmdy[i] * x[i] >= &val1agg. * &val2agg. * 1 ;
  
    		solve;
		
			create data DDE.opt_&i.(replace=yes rename = (i=USUBJID)) from [i] x DMDY AGE ;
		quit;
  	%end;
%mend;
